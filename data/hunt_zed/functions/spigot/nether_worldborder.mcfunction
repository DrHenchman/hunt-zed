...
execute store result storage hunt_zed:dynamic_tp Pos[0] double 0.125 run data get entity @s Pos[0]
execute store result storage hunt_zed:dynamic_tp Pos[1] double 1 run data get entity @s Pos[1]
execute store result storage hunt_zed:dynamic_tp Pos[2] double 0.125 run data get entity @s Pos[2]

execute as @e[type=minecraft:armor_stand,tag=nethercenter] run function hunt_zed:spigot/nether_worldborder_set
